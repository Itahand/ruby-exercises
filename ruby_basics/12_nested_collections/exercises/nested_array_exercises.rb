...
def count_empty_seats(chart)
  ...
  chart.flatten.reduce(0) { |sum, seat| (seat == nil) ? sum + 1 : sum }
...
def find_favorite(array_of_hash_objects)
  ...
  # Check if the current Hash[:is_my_favorite?] returns true
  (hash[:is_my_favorite?]) ? result = hash : result = result;
  end
